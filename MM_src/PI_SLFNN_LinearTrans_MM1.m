...
for i = 1:RPnoRuns
    ...
    %% precompute RHS & 2nd ders to save time from MMode F(z)=dzdt
    ...
    if firstTrain
        ...
        options=optimoptions(@lsqnonlin,'Display','none','MaxIter',2000,'TolFun',1e-6,...
    ...
    %
    %% SLFNN train error and test error 
...
% save learned_SLFNN_MM1 bestLearned;
...
z1All = allData(2,:);
sQSSAMSE = mse(sQSSA_SIM,z1All);                             %% z1_sQSSA - z1
rQSSAMSE = mse(rQSSA_SIM,z1All);                             %% z1_rQSSA - z1
PEAcMSE = mse(PEA_SIM,z1All);                                %% z1_PEA - z1
CSPc11MSE = mse(CSPc11_SIM,z1All);                           %% z1_CPS11c - z1
CSPs11MSE = mse(CSPs11_SIM,z1All);                           %% z1_CPS11s - z1
if ~fImpNR           %% implicit errors 
    CSPeMSE = mse(CSPe_SIM,zeros(size(CSPe_SIM)));           %% h_CSPe(z1,z2)-0
    CSPc21MSE = mse(CSPc21_SIM,zeros(size(CSPc21_SIM)));     %% h_CSPc21(z1,z2)-0
    CSPs21MSE = mse(CSPs21_SIM,zeros(size(CSPc21_SIM)));     %% h_CSPs21(z1,z2)-0
else                 %% explicit with NR
    CSPeMSE = mse(CSPe_SIM,z1All);
    CSPc21MSE = mse(CSPc21_SIM,z1All);
    CSPs21MSE = mse(CSPs21_SIM,z1All);
end
sQSSALinf = norm(sQSSA_SIM-z1All,Inf);
rQSSALinf = norm(rQSSA_SIM-z1All,Inf); 
PEAcLinf = norm(PEA_SIM-z1All,Inf);
CSPc11Linf = norm(CSPc11_SIM-z1All,Inf);
CSPs11Linf = norm(CSPs11_SIM-z1All,Inf);
if ~fImpNR
    CSPeLinf = norm(CSPe_SIM,Inf);
    CSPc21Linf = norm(CSPc21_SIM,Inf);
    CSPs21Linf = norm(CSPs21_SIM,Inf);
else
    CSPeLinf = norm(CSPe_SIM-z1All,Inf);
    CSPc21Linf = norm(CSPc21_SIM-z1All,Inf);
    CSPs21Linf = norm(CSPs21_SIM-z1All,Inf);
end
sQSSAL2 = norm(sQSSA_SIM-z1All,2);
rQSSAL2 = norm(rQSSA_SIM-z1All,2);
PEAcL2 = norm(PEA_SIM-z1All,2);
CSPc11L2 = norm(CSPc11_SIM-z1All,2);
CSPs11L2 = norm(CSPs11_SIM-z1All,2);
if ~fImpNR
    CSPeL2 = norm(CSPe_SIM,2);
    CSPc21L2 = norm(CSPc21_SIM,2);
    CSPs21L2 = norm(CSPs21_SIM,2);
else
    CSPeL2 = norm(CSPe_SIM-z1All,2);
    CSPc21L2 = norm(CSPc21_SIM-z1All,2);
    CSPs21L2 = norm(CSPs21_SIM-z1All,2);    
end
...
function [learnables, netDim] = prepareNetwork(inSz,noHL,hlSz,outSz,Mfast,UVin)
    ...
    %% NN: input slow variables y, output fast variables x
    ...
    learnables = [C1_a C1_b D2_c D2_d...
