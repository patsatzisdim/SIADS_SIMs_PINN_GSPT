%% Script for all visualizations of fCSI with the original variables
clc
clf
clear
...
fProj = 1;       % 1 or 2 for showing the projection to the 1st or 2nd fast variable
...
if fPlot==1
    ...
    e0 = 50;
    % domain Omega
    DomC = [1e-5 50; 1e-5 30; 1e-3 50; 1e-3 30];           
    % initial conditions (ICs)
    sv1_grid = linspace(50,150,5);   % ICs for s1 for uniform grid, more extended thatn test Data to show better boundaries
    sv2_grid = linspace(50,150,5);   % ICs for s2 for uniform grid
    ...
    if (fProj == 1) || (fProj == 2)
        y_init1 = [40; 1e-4; 35; 1e-2];
        y_init2 = [0.03; 1e0; 1.5; 10];
    end
    ...
    if fProj == 1
        ax.XLabel.String = '$s_1$';
        ax.YLabel.String = '$s_2$';
    ...
    if fProj == 1
        ...
        ax.ZLabel.Position(2) = 200;
        ax.ZLabel.Position(3) = 0.15;
    elseif fProj == 2
        ax.XLabel.Position(1) = 0.007;
        ax.XLabel.Position(2) = 2e-3;  
        ax.XLabel.Position(3) = 1e-4;
        ax.YLabel.Position(1) = 30;
        ax.YLabel.Position(2) = 0.15;
        ax.YLabel.Position(3) = 1e-4;
        ax.ZLabel.Position(1) = 7e-7;
    ...
    % print -depsc -tiff -r300 -painters ../../../Figs_GSPT/CompInhSt_SIM1.eps
    % much smaller for fast compile
    % print -dpng -r300 -painters ../../../Figs_GSPT/paper/CompInhSt_SIM1_l.png
...
elseif fPlot == 2
    ...
    return
    % huge, but good quality figure
    % print -depsc -tiff -r300 -painters ../../../Figs_GSPT/CompInhSt_DataSets1.eps
    % much smaller for fast compile
    % print -dpng -r300 -painters ../../../Figs_GSPT/light(png)_versions/CompInhSt_DataSets1_l.png
...
elseif fPlot == 3
    ...
    elseif fProj == 2
        ...
        ax.XLabel.Position(3) = 1e-4;
        ax.YLabel.Position(1) = 30;
        ax.YLabel.Position(2) = 0.25;
        ax.YLabel.Position(3) = 1e-4;
        ax.ZLabel.Position(1) = 7e-7;
        ax.ZLabel.Position(2) = 1e-4;
        ax.ZLabel.Position(3) = 0.7; 
    end
    ax.Position  = prePosition;
    set(gcf,'PaperPositionMode',"auto");
    ...
    return
    % huge, but good quality figure
    % print -depsc -tiff -r300 -painters ../../../Figs_GSPT/CompInhSt_AE1_PIML.eps
    % much smaller for fast compile
    % print -dpng -r300 -painters ../../../Figs_GSPT/light(png)_versions/CompInhSt_AE1_PIML_l.png
